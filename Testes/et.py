...
def formataComando(dir_tst, dir_saida, prog,arq, extraArgs="",ecpm=True,comVias=True,comBd=True,criarBd=True, 
                   nomebd=""): 
    path,barq = os.path.split(arq)
    nomearq,extarq = os.path.splitext(barq)
    bed = " -e %s" % (dir_tst)   
    bsd = " -o %s" % (dir_saida)
...
def abreZip(dest_dir,zip_dir, zip_file,subdirs=[]):
    ...
    with zipfile.ZipFile(zip_file,'r') as zf:
        zf.testzip()
        nomeDirDef,z = os.path.splitext(zip_file)
        daln = nomeDirDef
...
def executa(ecpm=False,comVias=False, comBd=False,subdirs=[]):
...
#     sufixo = ""
    ...
    arqgeo = "a.geo"
    try:
        opts, args = getopt.getopt(sys.argv[1:], "zZcthp:Is:E:P:O:U:X:g:", [])
        for opt, arg in opts:
            if opt == '-h':
                uso()
                sys.exit()
...
executa(False,False,False,[])

# Seja jmsilva.zip o arquivo a ser entregue
#    - colocar este arquivo no diretorio zips
#    - colocar os casos de testes no diretorio t
#
# Suponha que voce queira executar os testes referentes ao arquivo t1.geo
#
# para criar os diretorios t, zips, alunos: python et.py -I
#     - coloque jmsilva.zip no diretorio zips

# para unzipar todos .zip:   python et.py -Z -c -t
# para executar t1.geo e os respectivos .qry: python et.py -z -p [jmsilva] -g t1.geo
